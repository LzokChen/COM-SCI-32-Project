...
int main()
{
	...
	// destruction time.
	{
		...
		for (vector<Store>::iterator it = auxStores.begin(); it != auxStores.end(); it++)
			StorePtr.push_back(&(*it));
		
		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareStorePtr as the ordering relationship.
		sort(StorePtr.begin(), StorePtr.end(), &compareStorePtr);			//note: in here the pointer in StorePtr are sorted,
																			//but the Stores in auxStores are not sorted
		...
		vector<Store>::iterator it = stores.begin();
		while (it != stores.end())	//delete all Store in stores
		{
			stores.erase(it);
			it = stores.begin();
		}

		for (vector<Store*>::iterator it = StorePtr.begin(); it != StorePtr.end(); it++)
		{
			stores.push_back(*(*it));	
		}
